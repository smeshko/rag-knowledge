...
`synthetic_recipes/cookbooks/` is a special case: it is a *real* fixture set
generated by `rag-evals fixtures cut` (below) from copyrighted cookbook PDFs,
and it is **gitignored** — this repository is public, so the excerpts stay on
the machine that cut them. The cutter and the page ranges are committed, so the
set is reproducible by anyone holding the same PDFs. See doc 12 § 6 "Copyright
note".
...
| Real-cookbook excerpts (`data/fixtures/synthetic_recipes/cookbooks/`) | **gitignored** — public repo; regenerate with `rag-evals fixtures cut` |
